 ...
 *
 * Main Responsibilities: - Provides standard CRUD (Create, Read, Update,
 * Delete) operations for the token blacklist. - Allows the security filter to
 * efficiently check if a JWT has been blacklisted.
 *
 * Workflow: - When a user logs out, the `LogoutService` saves their JWT to this
 * repository. - The `JwtAuthenticationFilter` queries this repository for every
 * incoming request to ensure the token has not been invalidated.
 */
